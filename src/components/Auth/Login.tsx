...
const Navigate = useNavigate()
  ...
  const loginFormSubmit: SubmitHandler<LoginSchemaType> = async ({email, password}) => {

    const logindata = {
      email, password ,role:activeUser
    }

    const loginApiResponse = await loginService(logindata);
    let errorMessage = loginApiResponse?.response?.data?.message;

    console.log("loginApiResponse@@@@",loginApiResponse.data.user)
    if (loginApiResponse.status === 200) {
      toast.success("Login successful!");
      Cookies.set('tokensss', 'hdgajgdashdjsahdkashjdk',);
      Navigate("/")
    } else {
...
const selectActiveUser = (userType) => {
